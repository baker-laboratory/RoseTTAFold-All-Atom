...
from rf2aa.loss.loss import (calc_lj_grads, calc_chiral_grads)
...
class PositionalEncoding2D(nn.Module):
    ...
    def forward(self, seq, idx, bond_feats, dist_matrix, same_chain=None):
        ...
        res_dist, atom_dist = get_res_atom_dist(idx,
                                                bond_feats,
                                                dist_matrix,
                                                sm_mask,
        ...
        if self.use_same_chain and not self.enable_same_chain and same_chain is not None:
            emb_c = self.emb_chain(same_chain.long())
            out += emb_c * 0  # cursed but exists for backwards compatibility
        elif self.enable_same_chain:
            emb_c = self.emb_chain(same_chain.long())
            out += emb_c
...
class MSAPairStr2MSA(nn.Module):
    ...
    def forward(self, msa, pair, rbf_feat, state):
        ...
        msa = msa.index_add(1, torch.tensor([
            0,
...
class PairStr2Pair(nn.Module):
    ...
    def subblock(self, OP, pair, rbf_feat, crop):
        ...
        for i in range(ncrops):
            ...
            pairIdx[:, :(crop // 2)] = torch.arange(
                crop // 2, dtype=torch.long, device=pair.device) + idx[0, i:(i + 1), None] * (crop//2) - offsetN[:, None]
            pairIdx[:, (crop // 2):] = torch.arange(
                crop // 2, dtype=torch.long, device=pair.device) + idx[1, i:(i + 1), None] * (crop//2) - offsetC[:, None]
    ...
    def forward(self, pair, rbf_feat, state, crop=-1, is_prot=None):
        B, L = pair.shape[:2]
        ...
        if (crop > 0 and crop <= L):
            pair = self.subblock(lambda x, y: self.drop_row(self.tri_mul_out(x)), pair, rbf_feat, crop)

            pair = self.subblock(lambda x, y: self.drop_row(self.tri_mul_in(x)), pair, rbf_feat, crop)

            pair = self.subblock(lambda x, y: self.drop_row(self.row_attn(x, y)), pair, rbf_feat, crop)

            pair = self.subblock(lambda x, y: self.drop_col(self.col_attn(x, y)), pair, rbf_feat, crop)
...
class Str2Str(nn.Module):
    ...
    def forward(self,
        ...
        neighbor = get_seqsep_protein_sm(idx, bond_feats, dist_matrix, rotation_mask, cyclize=None)
...
class IterBlock(nn.Module):
    def __init__(
        ...
        self.pos = PositionalEncoding2D(d_rbf,
                                        minpos=minpos,
                                        maxpos=maxpos,
    ...
    def forward(self,
        ...
        if self.detach_xyz:
            xyz = xyz.detach()
        if use_checkpoint:
            msa = checkpoint.checkpoint(create_custom_forward(self.msa2msa),
                                        msa,
                                        pair,
                                        rbf_feat,
                                        state,
            ...
            pair = self.pair2pair(pair, rbf_feat, state, crop, is_prot=is_prot)
        ...
        return msa, pair, xyz, state, alpha, quat
...
class IterativeSimulator(nn.Module):
    ...
    def forward(self,
        ...
        for i_m in range(self.n_extra_block):
            ...
            msa_full, pair, xyz, state, alpha, quat = self.extra_block[i_m](
                msa_full,
                pair,
                xyz,
                state,
        ...
        for i_m in range(self.n_main_block):
            ...
            if self.use_chiral_l1:
                dchiraldxyz, = calc_chiral_grads(xyz.detach(), chirals)
                extra_l1 = dchiraldxyz[0].detach()
            msa, pair, xyz, state, alpha, quat = self.main_block[i_m](
                msa,
                pair,
                xyz,
                state,
        ...
        for i_m in range(self.n_ref_block):
            with ExitStack() as stack:
                ...
                xyz, state, alpha, quat = self.str_refiner(msa.float(),
                                                           pair.float(),
                                                           xyz.detach().float(),
                                                           state.float(),
                                                           idx,
        ...
        #         extra_l1 = None
        #
        #         xyzallatom, sta= self.finetune_refiner(
        #             seq_unmasked,
        ...
        return msa, pair, xyz, alpha_s, xyzallatom_s, state, quat
