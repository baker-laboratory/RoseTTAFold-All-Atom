...
from rf2aa.model.layers.AuxiliaryPredictor import (DistanceNetwork, MaskedTokenNetwork, LDDTNetwork, PAENetwork,
                                                   BinderNetwork)
...
class LegacyRoseTTAFoldModule(nn.Module):
    def __init__(
        ...
        self.full_emb = Extra_emb(d_msa=d_msa_full, d_init=ChemData().NAATOKENS - 1 + 4, p_drop=p_drop)
    ...
    def forward(
        ...
        if self.verbose_checks:
            ...
            msa_latent_indel2 = np.r_[2 * ChemData().NAATOKENS + NINDEL:2 * ChemData().NAATOKENS + NINDEL + NINDEL]
            ...
            for i, name in i_name:
                ...
                msa_full_indel = msa_full[0, 0, i, np.r_[ChemData().NAATOKENS:ChemData().NAATOKENS + NINDEL]]
                ...
                msa_latent_indel1 = msa_latent[0, 0, i, np.r_[2 * ChemData().NAATOKENS:2 * ChemData().NAATOKENS + NINDEL]]
                msa_latent_indel2 = msa_latent[0, 0, i, np.r_[2 * ChemData().NAATOKENS + NINDEL:2 * ChemData().NAATOKENS +
                                                              NINDEL + NINDEL]]
                msa_latent_term = msa_latent[0, 0, i, np.r_[2 * ChemData().NAATOKENS + 2*NINDEL:NMSAMASKED]]
        ...
        #    same_chain = None
        msa_latent, pair, state = self.latent_emb(msa_latent, seq, idx, bond_feats, dist_matrix, same_chain=same_chain)
        ...
        msa, pair, xyz, alpha_s, xyz_allatom, state, quat = self.simulator(seq_unmasked,
                                                                           msa_latent,
                                                                           msa_full,
                                                                           pair,
                                                                           xyz[:, :, :3],
        ...
        if self.verbose_checks:
            ...
            ic(pseq_0.shape)
            pseq_0 = pseq_0[0]
            ic(f"motif    sequence: { rf2aa.util.seq2chars(torch.argmax(pseq_0[is_motif], dim=-1).tolist())}"  # rf2aa.chemical.seq2chars is not callable
               )
            ic(f"diffused sequence: { rf2aa.util.seq2chars(torch.argmax(pseq_0[~is_motif], dim=-1).tolist())}")
        ...
        if self.get_quaternion:
            return (logits, logits_aa, logits_pae, logits_pde, p_bind, xyz, alpha_s, xyz_allatom, lddt, msa[:, 0], pair,
                    state, quat)
        else:
            return (logits, logits_aa, logits_pae, logits_pde, p_bind, xyz, alpha_s, xyz_allatom, lddt, msa[:, 0], pair,
                    state)
